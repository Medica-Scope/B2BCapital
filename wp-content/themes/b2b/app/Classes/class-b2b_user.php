<?php
     ...
     * @Description: This file contains the definition of the B2b_User class.
    ...
    class B2b_User
    {
        ...
        /**
         * Constructs a new instance of the B2b_User class.
         *
         * @version 1.0
         * @since 1.0.0
         * @package b2b
        ...
        /**
         * Retrieves the instance of the B2b_User class.
         *
         * @return B2b_User The instance of the B2b_User class.
         *
         * @version 1.0
         * @since 1.0.0
         * @package b2b
        ...
        /**
         * Retrieves the current user as a B2b_User object.
         *
         * @return B2b_User The current user.
         *
         * @version 1.0
         * @since 1.0.0
         * @package b2b
         ...
         *
         * @return B2b_User|int|WP_Error The inserted user object or an error object.
         * @throws \Exception
         *
         * @version 1.0
         * @since 1.0.0
        ...
        public function insert(): B2b_User|int|WP_Error
        {
            ...
            if (username_exists($this->username)) { // Check if the username already exists.
                $error->add('username_exists', __('Sorry, this phone number already exists!', 'b2b'), [
                    'status'  => FALSE,
            ...
            $profile         = new B2b_Profile(); // Create a new B2b_Profile object.
            ...
            do_action(B2b::_DOMAIN_NAME . "_after_create_user", $this); // Trigger an action after user creation.
         ...
         *
         * @return \B2B\APP\CLASSES\B2b_User|\WP_Error The updated user object or an error object.
         *
         * @version 1.0
         * @since 1.0.0
         * @package b2b
         ...
         *
         * @throws \Exception
         * @return \B2B\APP\CLASSES\B2b_User|\WP_Error|$this The current user object, an error object, or $this.
         *
         * @version 1.0
         * @since 1.0.0
         * @package b2b
         * @author Mustafa Shaaban
         */
        public function forgot_password(string $user_email_phone): B2b_User|WP_Error
        {
            ...
            if ($user) {
                ...
                if ($verification_type === B2b_User::VERIFICATION_TYPES['mobile']) {
                    ...
                    $this->send_phone_otp_code(B2b_User::VERIFICATION_TYPES['mobile'], $user->user_login);
                    ...
                    $this->send_whatsapp_otp_code(B2b_User::VERIFICATION_TYPES['whatsapp'], $user->user_login);
                } else {
        ...
        public static function check_reset_code($key): bool|WP_Error
        {
            ...
            $decrypt_data = B2b_Cryptor::Decrypt($key); // Decrypt the reset code
         ...
         * @package b2b
         * @author Mustafa Shaaban
         * @return \B2B\APP\CLASSES\B2b_User The B2b User object
         */
         ...
         * @package b2b
         * @author Mustafa Shaaban
         * @return \B2B\APP\CLASSES\B2b_User|\WP_Error The B2b User object if found, otherwise returns a WP_Error object
         */
        public static function get_user_by(string $field, string $value): B2b_User|WP_Error
        {
            ...
            if ($user) {
                return self::get_user($user); // Get the B2b User object
            } else {
                // The user does not exist
                $error->add('invalid_user', __("This user does not exist!.", 'b2b'), [
                    'status'  => FALSE,
                    'details' => [
                        'user'  => $value,
                        'field' => $field
        ...
        /**
         * Convert the default WP user object to a B2b User object
         *
         ...
         * @package b2b
         * @author Mustafa Shaaban
         * @return \B2B\APP\CLASSES\B2b_User The converted B2b User object
         */
        private function convert(WP_User $user): B2b_User
        {
            $class    = __CLASS__;
            $new_user = new $class(); // Create a new B2b User object
        ...
        /**
         * Assign WP_User properties to B2b_User
         *
        ...
        protected function login(): B2b_User|WP_Error
        {
            ...
            if (empty($user)) {
                ...
                if (empty($user)) {
                    ...
                    $error->add('invalid_username', __("Your login credentials are invalid.", 'b2b'), [
                        'status'  => FALSE,
            ...
            if (!empty($user)) {
                $this->assign($user); // Assign properties from WP_User to B2b_User

                $check_pwd = wp_check_password($user_password, $this->password, $this->ID); // Check if the password is valid

                if (!$check_pwd) {
                    // The login credentials are invalid
                    $error->add('invalid_password', __("Your login credentials are invalid.", 'b2b'), [
                        'status'  => FALSE,
                ...
                if (!$profile_id) {
                    ...
                    $error->add('invalid_profile', __("This account is temporarily disabled or blocked. Please contact us.", 'b2b'), [
                        'status' => FALSE
                    ]);
                    return $error; // Return the WP_Error object
                }
                ...
                if (!isset($profile->taxonomy['industry']) || empty($profile->taxonomy['industry'])) {
                    ...
                    $error->add('empty_industry', __("You have to use at least 1 industry.", 'b2b'), [
                        'status'  => FALSE,
        ...
        public function setup_verification(string $type): WP_Error|bool
        {
            ...
            if ($this->user_meta['verification_type'] === self::VERIFICATION_TYPES['mobile']) {
                ...
                if (is_wp_error($verification)) {
                    ...
                    $error->add('mobile_error', __($verification->get_error_message(), 'b2b'), [
                        'status'  => FALSE,
                        'details' => [
                            'e' => '',
                        ]
                ...
                if (!$verification) {
                    ...
                    $error->add('email_error', __("The verification code didn't send!", 'b2b'), [
                        'status'  => FALSE,
                        'details' => [
        ...
        public function send_phone_otp_code(string $type, string $to = ''): WP_Error|bool
        {
            ...
            if ($type === 'authentication') {
                ...
                $send = $this->send_by_twilio([
                    'to'   => '+' . $to,
                    'from' => '+19894738633',
                    'body' => sprintf(__('Your authentication code for B2B is %s', 'b2b'), $randomNumber)
                ]);
                ...
                $send = $this->send_by_twilio([
                    'to'   => '+' . $to,
                    'from' => '+19894738633',
                    'body' => sprintf(__('Your verification code for B2B is %s', 'b2b'), $randomNumber)
                ]);
        ...
        private function send_by_twilio(array $data): \stdClass|\WP_Error
        {
            ...
            if (curl_errno($ch)) {
                ...
                $error->add('sending_error', __("Response Error!", 'b2b'), [
                    'status'  => FALSE,
                    'details' => [
