...
$active_link  = $current_slug;


// 	var_dump( $active_link );
// }
?>
<div class="landing-page main">
	<div class="back-texture d-none d-xl-flex">
		<img src="<?= Nh_Hooks::PATHS['public']['img']; ?>/landing-page/texture.webp" alt="Texture" class="img-fluid">
	</div>
	<!-- App Sidebar -->
...
//			}
		...
		<ul class="navbar-nav app-navigation">
			<li class="nav-item <?= $active_link === 'contact-us' ? 'active' : ''; ?>">
				<a class="nav-link"
					href="<?= apply_filters( 'nhml_permalink', get_permalink( get_page_by_path( 'contact-us' ) ) ); ?>">
					<?= __( 'Contact Us', 'ninja' ); ?>
				</a>
			</li>
			<li class="nav-item <?= $active_link === 'about' ? 'active' : ''; ?>">
				<a class="nav-link"
					href="<?= apply_filters( 'nhml_permalink', get_permalink( get_page_by_path( 'about' ) ) ); ?>">
					<?= __( 'About', 'ninja' ); ?>
				</a>
			</li>
			<li class="nav-item <?= $active_link === 'service' ? 'active' : ''; ?>">
				<a class="nav-link" href="<?= get_post_type_archive_link( 'service' ); ?>">
					<?= __( 'Services', 'ninja' ); ?>
				</a>
			</li>
		</ul>
			...
			<header id="masthead" class="site-header app-header">
				<ul class="social-links">
					<li class="social-item">
						<a href="<?= Nh_Init::$_NH_CONFIGURATION['social']['ninja_social_in']; ?>" target="_blank"
							class="social-link"><i class="social-icon bbc-linkedin"></i></a>
					</li>
					<li class="social-item">
						<a href="<?= Nh_Init::$_NH_CONFIGURATION['social']['ninja_social_fb']; ?>" target="_blank"
					...
					if ( is_user_logged_in() ) {
						...
						<div class="offcanvas offcanvas-start" data-bs-backdrop="false" tabindex="-1" id="staticBackdrop"
							...
							<div class="offcanvas-header">
								...
								<button type="button" class="btn btn-outline-light" data-bs-dismiss="offcanvas" aria-label="Close">X
								</button>
							</div>
							<div class="offcanvas-body">
								<ul class="navbar-nav app-navigation">
									<li class="nav-item mb-2  <?= $active_link === 'dashboard' ? 'active' : ''; ?>">
										<a class="nav-link text-white"
											href="<?= apply_filters( 'nhml_permalink', get_permalink( get_page_by_path( 'dashboard' ) ) ) ?>">
											<?= __( 'Dashboard', 'ninja' ); ?>
										</a>
									</li>
									<li class="nav-item mb-2 <?= $active_link === 'contact-us' ? 'active' : ''; ?>">
										<a class="nav-link text-white"
											href="<?= apply_filters( 'nhml_permalink', get_permalink( get_page_by_path( 'contact-us' ) ) ); ?>">
											<?= __( 'Contact Us', 'ninja' ); ?>
										</a>
									</li>
									<li class="nav-item mb-2 <?= $active_link === 'about' ? 'active' : ''; ?>">
										<a class="nav-link text-white"
											href="<?= apply_filters( 'nhml_permalink', get_permalink( get_page_by_path( 'about' ) ) ); ?>">
											<?= __( 'About', 'ninja' ); ?>
										</a>
									</li>
									<li class="nav-item mb-2 <?= $active_link === 'service' ? 'active' : ''; ?>">
										<a class="nav-link text-white" href="<?= get_post_type_archive_link( 'service' ); ?>">
											<?= __( 'Services', 'ninja' ); ?>
										</a>
									</li>
									<li class="nav-item mb-2 <?= $active_link === 'faq' ? 'active' : ''; ?>">
										<a class="nav-link text-white" href="<?= get_post_type_archive_link( 'faq' ); ?>">
											<?= __( 'FAQs', 'ninja' ); ?>
										</a>
									</li>
